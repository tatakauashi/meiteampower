...
SELECT t.member_id, 
	...
	SELECT sm.member_id, count(*) AS cnt FROM Stage_View s JOIN Stage_Member sm ON (s.stage_id = sm.stage_id AND s.revision = sm.revision)
	WHERE NOT EXISTS (SELECT 'x' FROM Program p WHERE p.program_id = s.program_id AND p.program_id = 51)
	  AND s.stage_date <= '2016-12-25'
	  AND s.delete_time is NULL
	GROUP BY sm.member_id
) t
JOIN Member m ON (m.member_id = t.member_id) JOIN Belonging b ON (b.member_id = m.member_id)
WHERE b.from_date <= '2016-12-25' AND b.to_date >= '2016-12-25' AND b.team_id <> 100
ORDER BY `残り公演数`, `区切りの回数` DESC, b.team_id, m.sort_order;


SELECT sm.member_id, m.member_name, count(*) AS cnt
FROM Stage_View s
JOIN Stage_Member sm ON (sm.stage_id = s.stage_id AND sm.revision = s.revision)
JOIN Member m ON (m.member_id = sm.member_id)
WHERE s.stage_date between '2008-10-21' AND '2016-11-06' AND sm.member_id = 58 AND s.program_id <> 51;
